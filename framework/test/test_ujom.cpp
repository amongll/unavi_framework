...
int main(int arg,char* argv[])
{
	...
	::inet_pton(AF_INET,"127.0.0.1",&dst.sin_addr.s_addr);
	cycle->thread_start();

	urtmfp::URtmfpPeerId* peerid = new urtmfp::URtmfpPeerId((sockaddr*)&dst);
	ujom::UJOMCmdArg* start_arg = new ujom::UJOMCmdArg("method2", "objtest");

	UOMBSvc::run_proxy(proto_id,
		UOMBSvcDeclare<ujom::UJOMExmpleA>::first_sign(),
		&ujom::JOMA_CX_proxy::class_sign,
		peerid,
		start_arg,
		true,
		true
